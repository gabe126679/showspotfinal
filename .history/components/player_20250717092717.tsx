...
export const MusicPlayerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const playSong = async (song: Song, playlist: Song[] = []) => {
    try {
      ...
      // Get the public URL for the song file from Supabase
      let audioUrl = song.song_file;
      
      // If it's a Supabase storage path, get the public URL
      if (song.song_file && !song.song_file.startsWith('http')) {
        const { data } = supabase.storage
          .from('songs')
          .getPublicUrl(song.song_file);
        audioUrl = data.publicUrl;
      }

      console.log('Playing song:', song.song_title, 'URL:', audioUrl);
      ...
      Alert.alert('Error', 'Could not play this song. Please check the audio file.');
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.playerContent}>
        ...
        <View style={styles.albumArtContainer}>
          <Image
            source={{ 
              uri: currentSong.song_image && currentSong.song_image.startsWith('http') 
                ? currentSong.song_image 
                : supabase.storage.from('song-images').getPublicUrl(currentSong.song_image).data.publicUrl
            }}
