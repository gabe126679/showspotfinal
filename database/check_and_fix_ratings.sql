-- First, let's check the actual structure of the individual_ratings table
-- Run this query first to see what columns exist:
-- SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'individual_ratings';

-- Based on the error, let's try with the most common column names
-- The individual_ratings table likely has one of these column names for the user:
-- - rater_id
-- - user_id  
-- - spotter_id (but you said this doesn't exist)

-- Let's create the functions with 'rater_id' since that's most logical for a ratings table
...
BEGIN
  RETURN EXISTS (
    ...
    WHERE ir.entity_id = user_has_rated_entity.entity_id 
    AND ir.entity_type = user_has_rated_entity.entity_type
    AND ir.rater_id = user_has_rated_entity.user_id
  );
END;
$$;
...
BEGIN
  ...
  FROM ratings r
  WHERE r.entity_id = get_entity_rating.entity_id 
  AND r.entity_type = get_entity_rating.entity_type;
...
BEGIN
  ...
  INTO user_rating
  FROM individual_ratings ir
  WHERE ir.entity_id = get_user_rating.entity_id 
  AND ir.entity_type = get_user_rating.entity_type
  AND ir.rater_id = get_user_rating.user_id;
...
BEGIN
  ...
  SELECT user_has_rated_entity(rate_entity.entity_id, rate_entity.entity_type, rate_entity.user_id)
  INTO already_rated;
  ...
  VALUES (rate_entity.entity_id, rate_entity.entity_type, rate_entity.user_id, rate_entity.rating_value);
  ...
  FROM individual_ratings ir
  WHERE ir.entity_id = rate_entity.entity_id 
  AND ir.entity_type = rate_entity.entity_type;

  -- Insert or update the ratings table
  INSERT INTO ratings (entity_id, entity_type, current_rating, total_raters)
  VALUES (rate_entity.entity_id, rate_entity.entity_type, new_average, new_count)
