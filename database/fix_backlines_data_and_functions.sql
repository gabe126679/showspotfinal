-- Fix backlines data type issues and functions
-- This addresses the "malformed array literal" error

-- First, let's fix any existing data that has string arrays instead of JSONB arrays
UPDATE shows 
SET show_backlines = '[]'::jsonb 
WHERE show_backlines IS NULL 
   OR show_backlines = '"[]"'::jsonb 
   OR (show_backlines::text = '[]' AND pg_typeof(show_backlines) = 'jsonb'::regtype);

-- Ensure the column is proper JSONB type
ALTER TABLE shows 
ALTER COLUMN show_backlines TYPE JSONB USING 
CASE 
  WHEN show_backlines IS NULL THEN '[]'::jsonb
  WHEN pg_typeof(show_backlines) != 'jsonb'::regtype THEN 
    CASE 
      WHEN show_backlines::text = '[]' THEN '[]'::jsonb
      ELSE show_backlines::jsonb
    END
  ELSE show_backlines
END;
...
ALTER TABLE shows 
ALTER COLUMN show_backlines SET DEFAULT '[]'::jsonb;

-- Now create the fixed functions with better error handling
...
BEGIN
  ...
  IF current_backlines IS NULL OR jsonb_typeof(current_backlines) != 'array' THEN
    RETURN FALSE;
  END IF;
  ...
  FOR i IN 0..(jsonb_array_length(current_backlines) - 1) LOOP
    ...
    IF (backline_obj->>'backline_artist')::UUID = v_backline_artist THEN
      RETURN COALESCE(backline_obj->'backline_voters', '[]'::jsonb) ? v_user_id::text;
    END IF;
  END LOOP;
