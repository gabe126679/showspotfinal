...
const SongUploadForm: React.FC<SongUploadFormProps> = ({
  ...
  const uploadFileToSupabase = async (file: any, bucket: string, path: string) => {
    try {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const songFilePath = await uploadFileToSupabase(
        songFile,
        "songs",
        `artist_${artistData.artistID}`
      );

      // Upload song image or use artist profile image
      let songImagePath = artistData.artistProfileImage;
      if (songImage) {
